    ...
    if menu == "📈 PERFORMANCE":
        ...
        with t_sub:
            ...
            if not df_f.empty:
                ...
                # Filter Menyamping (Rasio 3:3:1 agar box luas)
                f_cols = st.columns([10, 10]) 
                with f_cols[0]:
                    m_cat = st.multiselect("CAT:", sorted(df_f["CATEGORY"].unique()), key="m_sub_c")
                with f_cols[1]:
                    df_tmp = df_f[df_f["CATEGORY"].isin(m_cat)] if m_cat else df_f
                    m_sub = st.multiselect("SUBCAT:", sorted(df_tmp["SUBCATEGORY"].unique()), key="m_sub_s")
        ...
        with t_brand:
            ...
            if not df_f.empty:
                ...
                # Filter Menyamping (Rasio 3:3:3:0.5)
                f_cols = st.columns([5, 5, 5]) 
                with f_cols[0]:
                    m_cat = st.multiselect("CAT:", sorted(df_f["CATEGORY"].unique()), key="m_br_c")
                with f_cols[1]:
                    df_s = df_f[df_f["CATEGORY"].isin(m_cat)] if m_cat else df_f
                    m_sub = st.multiselect("SUBCAT:", sorted(df_s["SUBCATEGORY"].unique()), key="m_br_s")
                with f_cols[2]:
                    df_b = df_s[df_s["SUBCATEGORY"].isin(m_sub)] if m_sub else df_s
                    m_br = st.multiselect("BRAND:", sorted(df_b["BRAND"].unique()), key="m_br_b")
        ...
        with t_plu:
            ...
            if not df_f.empty:
                ...
                # 3. Filter Menyamping (Hanya 2 Filter: CAT & PLU)
                # Rasio [4, 5, 1] memberikan ruang ekstra luas untuk nama PLU
                f_cols = st.columns([5, 5])
                
                with f_cols[0]:
                    m_cat = st.multiselect("CAT:", sorted(df_f["CATEGORY"].unique()), key="m_plu_c")
                
                with f_cols[1]:
                    # Filter pilihan PLU berdasarkan Category yang dipilih
                    df_p_list = df_f[df_f["CATEGORY"].isin(m_cat)] if m_cat else df_f
                    m_plu = st.multiselect("PLU:", sorted(df_p_list["PLU"].unique()), key="m_plu_p")

                # 4. Logika Filter Data Akhir
                if m_cat: df_f = df_f[df_f["CATEGORY"].isin(m_cat)]
        ...
        with t_seg:
            ...
            if not df_seg.empty:
                # 1. SIAPKAN KONTAINER DI ATAS
                card_place = st.container()
                
                f1, f2, f3 = st.columns(3)
                with f1: s_cat = st.selectbox("FILTER CATEGORY ", ["ALL"] + sorted(df_seg["CATEGORY"].unique()), key="seg_c")
                with f2:
                    df_s = df_seg[df_seg["CATEGORY"] == s_cat] if s_cat != "ALL" else df_seg
                    s_sub = st.selectbox("FILTER SUBCATEGORY", ["ALL"] + sorted(df_s["SUBCATEGORY"].unique()), key="seg_s")
                with f3:
                    df_b = df_s[df_s["SUBCATEGORY"] == s_sub] if s_sub != "ALL" else df_s
                    s_brand = st.selectbox("FILTER BRAND", ["ALL"] + sorted(df_b["BRAND"].unique()), key="seg_b")
                ...
                if s_cat != "ALL": df_seg_f = df_seg_f[df_seg_f["CATEGORY"] == s_cat]
                if s_sub != "ALL": df_seg_f = df_seg_f[df_seg_f["SUBCATEGORY"] == s_sub]
                if s_brand != "ALL": df_seg_f = df_seg_f[df_seg_f["BRAND"] == s_brand]

                if not df_seg_f.empty:
                    # --- KARTU DIPANGGIL DI SINI (1 KALI SAJA) ---
                    # Pastikan variabel yang dimasukkan adalah df_seg_f (data yang sudah difilter)
                    with card_place:
                        render_performance_cards(df_seg_f, is_category=True)
                    
                    # 3. AGGREGASI DATA UNTUK TABEL
                    group_cols = ["SEGMENT_TYPE", "SEGMENT_VALUE", "CATEGORY", "SECTION"]
                    num_cols = df_seg_f.select_dtypes(include=[np.number]).columns.tolist()
                    agg_dict = {c: ('mean' if any(x in c for x in ['GROWTH', 'PENETRATION']) else 'sum') for c in num_cols}
                    df_clean = df_seg_f.groupby(group_cols).agg(agg_dict).reset_index()

                    # 4. RENDER SUB-TABS
                    seg_list = sorted(df_clean["SEGMENT_TYPE"].unique())
                    tabs_seg = st.tabs([s.upper() for s in seg_list])
                    
                    for i, s_type in enumerate(seg_list):
                        with tabs_seg[i]:
                            df_tab = df_clean[df_clean["SEGMENT_TYPE"] == s_type].copy()
                            # TAMPILKAN TABEL SAJA (Jangan panggil render_cards lagi di sini)
                            display_styled_table(reorder_final(df_tab, "category"))
                            render_static_affinity_matrix() # Tambahkan di baris terakhir tab
